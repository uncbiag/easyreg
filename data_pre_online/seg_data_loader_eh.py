...
class SegmentationDataset(Dataset):
    ...
    def __init__(self, data_path,is_train=True, transform=None):
        ...
        self.len_dataset = len(self.path_list)
        self.data_pq_record = Queue()
        if is_train:
            self.buffer_len = 800
        else:
            self.buffer_len = 1
    ...
    def __getitem__(self, idx):  # idx will not be used in training  case
        """
        :param idx: id of the items
        :return: the processed data, return as type of dic
        """
        cur_len = self.get_cur_pq_len()
        if cur_len ==0:
            if cur_len ==0:
                print("the buffer is emtpy now ")
            for i in range(self.buffer_len):
                #  in training phase the the random sampling is generated by randint here, in other phase, the given idx will be used
                if self.is_train:
                    idx = randint(0,self.len_dataset-1)
                dic = read_h5py_file(self.path_list[idx])
                self.data_pq_record.put((dic,idx))
            print("the buffer is filled now, the cur_sz is {} ".format(self.get_cur_pq_len()))

        dic, idx = self.data_pq_record.get()
