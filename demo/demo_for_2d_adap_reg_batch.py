...
def get_init_momentum_list(folder_path, name_list):
    return [os.path.join(os.path.join(folder_path,name),'m.pt') for name in name_list]
...
def nonp_optimization(si, moving,target,spacing,fname,l_moving=None,l_target=None, init_weight= None,expr_folder= None,mermaid_setting_path=None,moving_momentum=None):
    affine_map = None
    if si is not None:
        affine_map = si.opt.optimizer.ssOpt.get_map()

    si =  SI.RegisterImagePair()
    si.set_light_analysis_on(False)  # todo here should be False
    #extra_info=None
    ...
    # if moving_momentum is not None:
    #     si.set_model_parameters({'m':moving_momentum})
    ...
    si.register_images(moving, target, spacing, extra_info=extra_info, LSource=l_moving,
                            ...
                            nr_of_iterations=100,
                            visualize_step=30 , # todo here should be 30
...
def get_batch_input(img_pair_list, weight_pair_list,init_momentum_path_list):
    moving_list =[]
    target_list= []
    spacing=None
    moving_init_weight_list = [] if weight_pair_list else None
    target_init_weight_list = [] if weight_pair_list else None
    moving_init_momentum_list = [] if init_momentum_path_list else None
    for i, img_pair in enumerate(img_pair_list):
        moving, target, spacing, moving_init_weight, target_init_weight,moving_momentum\
            =get_input(img_pair,weight_pair=weight_pair_list[i] if weight_pair_list is not None else None,
                       momentum_pair=init_momentum_path_list[i] if init_momentum_path_list is not None else None)
        ...
        if init_momentum_path_list is not None:
            moving_init_momentum_list.append(moving_momentum)
    ...
    if init_momentum_path_list is not None:
        moving_momentum =torch.cat(moving_init_momentum_list,0)
    else:
        moving_momentum = None
    return moving, target, spacing, moving_init_weight, target_init_weight, moving_momentum


def get_input(img_pair,weight_pair=None,momentum_pair=None):
    ...
    if momentum_pair is not None:
        moving_momentum = torch.load(momentum_pair)
...
def do_pair_registration(pair_list, pair_name_list, weight_pair_list,do_affine=True,expr_folder=None,mermaid_setting_path=None,init_momentum_path_list=None):
    moving, target, spacing, moving_init_weight, _,moving_momentum = get_batch_input(pair_list, weight_pair_list,init_momentum_path_list)
    moving, target, moving_init_weight,moving_momentum = warp_data([moving, target, moving_init_weight,moving_momentum])
    si = None
    if do_affine:
    ...
    return_val = nonp_optimization(si, moving, target, spacing, pair_name_list, init_weight=moving_init_weight,
                                   expr_folder=expr_folder, mermaid_setting_path=mermaid_setting_path,moving_momentum=moving_momentum)
...
def demo_multi():
    ...
    parser.add_argument('--expr_name', required=False, default='non_name_tv_weighted_omt4_3',
    ...
    if not llf:
        ...
        if model_name == 'lddmm_adapt_smoother_map':
            mermaid_setting_path = '../mermaid/demos/sample_generation/cur_settings_adpt_lddmm_for_synth_tv4.json'
        elif model_name == 'lddmm_shooting_map':
            mermaid_setting_path = '../mermaid/demos/sample_generation/cur_settings_lddmm_for_synth_sgd.json'
        else:
        ...
        if args.mermaid_setting_path != 'non_path':
            mermaid_setting_path = args.mermaid_setting_path
        else:
            if model_name == 'lddmm_adapt_smoother_map':
                mermaid_setting_path = '../mermaid/demos/sample_generation/cur_settings_adpt_lddmm_for_synth_reverse.json'
            elif model_name == 'lddmm_shooting_map':
                mermaid_setting_path = '../mermaid/demos/sample_generation/cur_settings_lddmm_for_synth_sgd.json'
            else:
                mermaid_setting_path = '../mermaid/demos/sample_generation/cur_settings_svf_for_synth_sgd.json'
    ...
    if use_init_momentum:
        ...
        init_momentum_path_list = get_init_momentum_list(momentum_folder,pair_name_list)
    do_optimization = True  # todo make sure this is true in optimization
    do_evaluation = True
    run_grid_expr = False
    color_image = False
    weights_np = np.load('../tools/weights.npy')
    stds_np = np.load('../tools/stds.npy')
    if run_grid_expr:
        num_stds =15
        num_weights =10
    ...
    for i in range(num_stds):
        for j in range(num_weights):
            ...
            idx = i*num_weights + j
            stds = list(stds_np[i])
            weights = list(weights_np[idx])
            if model_name == 'lddmm_adapt_smoother_map':
                weights =None

            if do_optimization:
                get_mermaid_setting(mermaid_setting_path, expr_folder,modify=run_grid_expr,stds=stds,weights=weights)
                do_pair_registration(pair_path_list, pair_name_list, init_weight_path_list, do_affine=do_affine,
                                         expr_folder=expr_folder, mermaid_setting_path=mermaid_setting_path,init_momentum_path_list=init_momentum_path_list)
            if do_evaluation:
