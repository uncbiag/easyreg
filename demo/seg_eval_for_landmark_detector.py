...
import torch
from demo.task import ModelTask, DataTask
...
def run_segmentation(image_list, general_config):
    setting_folder_path = general_config["setting_folder_path"]
    task_output_path = general_config["task_output_path"]
    model_path = general_config["model_path"]
    gpu_id = general_config["gpu_id"]

    fname_list = [f.split('/')[-1].split('.')[0] for f in image_list]

    dm, tsm = init_test_env(setting_folder_path, task_output_path, image_list, fname_list)
