...
def debug_model(opt,model, dataloaders):
    ...
    if isinstance(model_path, list):
        for i, path in enumerate(model_path):
            __debug_model(opt,model,dataloaders,path,str(i)+'_')
    else:
        __debug_model(opt,model, dataloaders,model_path)




def __debug_model(opt,model,dataloaders, model_path,task_name=''):
    since = time()
