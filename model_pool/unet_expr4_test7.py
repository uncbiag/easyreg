...
class UNet3Dt7(nn.Module):
    def __init__(self, in_channel, n_classes):
        self.in_channel = in_channel
        self.n_classes = n_classes
        super(UNet3Dt7, self).__init__()
        ...
        self.ud2 = self.udecoder_2(128,160)
        ...
        self.ud1 = self.udecoder_1(128,160)
        ...
        self.dc1 = self.decoder(128, 160, kernel_size=3, stride=1, padding=1, bias=True, batchnorm=True)
        self.ud0 = self.udecoder_0(160)
        self.count =0
    ...
    def udecoder_2(self,in_channels,out_channels, bias=True):
        layer = nn.Sequential(
            ...
            # To Do  comment bias
            nn.Conv3d(out_channels, self.n_classes, 3, stride=1,padding=1, bias=True),
    ...
    def udecoder_0(self, in_channels):
        layer = nn.Sequential(
            nn.ConvTranspose3d(in_channels, self.n_classes, 3, stride=1,padding=1,bias=True),
            nn.BatchNorm3d(self.n_classes))
        return layer
    ...
    def gate_module(self, input1,input2,input3):
        ...
        return torch.max(softmax(input3,1),1,keepdim=True)[0]
