...
class UNet3DB7(nn.Module):
    def __init__(self, in_channel, n_classes):
        self.in_channel = in_channel
        self.n_classes = n_classes
        super(UNet3DB7, self).__init__()
        ...
        self.dc8 = self.decoder(128+128, 128, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.dc7 = self.decoder(128, 128, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.ud2 = self.udecoder_2(128,128)
        self.dc6 = self.decoder(128, 128, kernel_size=2, stride=2, bias=True, batchnorm=True)
        self.dc5 = self.decoder(128+128, 128, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.dc4 = self.decoder(128, 128, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
    ...
    def cmid(self, in_channels, out_channels, bias=True):
        layer = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, 1, stride=1,
                      padding=0, bias=bias),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_channels, out_channels, 1, stride=1,
