...
def udecoder_1(in_channels,out_channels, bias=True):
    layer = nn.Sequential(
        nn.ConvTranspose3d(in_channels, out_channels,  2, stride=2,bias=bias),
        nn.ReLU(inplace=True),
        nn.Conv3d(out_channels, 128, 3, stride=1, padding=1,bias=True),
        nn.Dropout3d(p=0.5, inplace=True))
    return layer

def udecoder_0(in_channels):
    layer = nn.Sequential(
        nn.ConvTranspose3d(in_channels,128, 3, stride=1,padding=1,bias=True),
        nn.Dropout3d(p=0.5, inplace=True))
    return layer
...
class UNet_Dis(nn.Module):
    def __init__(self, n_classes, bias=True, BN=True):
        ...
        self.ud0 = udecoder_0(128)
        self.adapt_conv = nn.Conv3d(128*3, n_classes, 1, stride=1)


        print("this is vonet_t9_dis")
