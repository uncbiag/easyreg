...
class UNet3Dt1(nn.Module):
    def __init__(self, in_channel, n_classes):
        self.in_channel = in_channel
        self.n_classes = n_classes
        super(UNet3Dt1, self).__init__()
        ...
        self.cc_low = self.cdecoder_7(128)
        ...
        self.cd_low = self.cdecoder_7(128)
        ...
        self.dc0_re = self.decoder_0(128)
    ...
    def cdecoder_7(self,in_channels):
        layer = nn.Sequential(
            ...
            # To Do  comment bias
            nn.Conv3d(128, self.n_classes, 1, stride=1, bias=False))
        return layer
    ...
    def decoder_0(self, in_channels):
        layer = nn.Sequential(
    ...
    def combine_conv(self):
        layer = nn.Sequential(
            nn.ConvTranspose3d(self.n_classes*4, self.n_classes*2, 1, stride=1,padding=0),
            nn.ReLU(inplace=True),
            nn.ConvTranspose3d(self.n_classes * 2, self.n_classes, 1, stride=1, padding=0)
        )
        return layer
    def AdaptWeightedLayer(self,input,n_class,n_map):
        """
        ...
        """
        permute_idx = [i+j*n_map for i in range(n_class) for j in range(n_map)]
        permute_input = input(torch.LongTensor(permute_idx))
    ...
    def forward(self, x):
        ...
        d1 = self.dc1(d2)
        d1 += d2
        dc_high = self.dc0_re(d1)  # 1. in 64 o 57
        del d3, d2
        #print(cc_low.shape,dc_high.shape,dc_mid.shape,dc_low.shape)

        d0 = torch.cat( (cc_low,dc_low ,dc_mid,dc_high),1)
        d0 = self.confid_conv(d0)
        # d0 = self.adapt_conv(d0,self.n_classes, 3)
        return d0
