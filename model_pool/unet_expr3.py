...
class UNet3D3(nn.Module):
    def __init__(self, in_channel, n_classes):
        ...
        self.dc9 = self.decoder(512, 512, kernel_size=2, stride=2, bias=True, batchnorm=False)
        self.dc8 = self.decoder(256 + 512, 256, kernel_size=3, stride=1, padding=1, bias=True, batchnorm=False)
        self.dc7 = self.decoder(256, 256, kernel_size=3, stride=1, padding=1, bias=True, batchnorm=False)
        self.cd_low = self.cdecoder_7(256)
        self.dc6 = self.decoder(256, 256, kernel_size=2, stride=2, bias=True, batchnorm=False)
        self.dc5 = self.decoder(128 + 256, 128, kernel_size=3, stride=1, padding=1, bias=True, batchnorm=False)
        self.dc4 = self.decoder(128, 128, kernel_size=3, stride=1, padding=1, bias=True, batchnorm=False)
        self.cd_mid = self.cdecoder_4(128)
        self.dc3 = self.decoder(128, 128, kernel_size=2, stride=2, bias=True, batchnorm=False)
        ...
        self.dc0_re = self.decoder_0(64, n_classes, kernel_size=1, stride=1, bias=False)
    ...
    def cdecoder_7(self,in_channels):
        layer = nn.Sequential(
            ...
            nn.ConvTranspose3d(128, 72, 2, stride=2, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv3d(72, 72, kernel_size=3, padding=1),
            nn.ConvTranspose3d(72, self.n_class, 2, stride=2, bias=True),
            nn.ReLU(inplace=True))
        return layer
    ...
    def cdecoder_4(self,in_channels):
        layer = nn.Sequential(
            nn.ConvTranspose3d(in_channels, 128, 2, stride=2,bias=True),
            nn.ReLU(),
            nn.ConvTranspose3d(128,  self.n_class, 2, stride=2, bias=True))
        return layer
