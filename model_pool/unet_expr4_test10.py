...
class UNet3Dt9_sim(nn.Module):
    # base on t8 add dropout
    def __init__(self, in_channel, n_classes):
        self.in_channel = in_channel
        self.n_classes = n_classes
        super(UNet3Dt9_sim, self).__init__()
        ...
        self.ud2 = self.udecoder_2(128, 32)
        ...
        self.ud1 = self.udecoder_1(128, 32)
        ...
        self.ud0 = self.udecoder_0(128,32,bias=True)
        self.adapt_conv = nn.Conv3d(32 * 3, n_classes, 1, stride=1)
        self.count = 0

        print("this is t9_sim")
    ...
    def udecoder_2(self,in_channels,out_channels, bias=True):
        layer = nn.Sequential(
            nn.ConvTranspose3d(in_channels, out_channels*2, 2, stride=2,bias=bias),
            nn.Dropout3d(p=0.5, inplace=True),
            nn.ReLU(inplace=True),
            nn.BatchNorm3d(out_channels*2),
            nn.ConvTranspose3d(out_channels*2, out_channels*2, 2, stride=2,bias=bias),
            nn.Dropout3d(p=0.5, inplace=True),
            #  To Do  maybe here should be exchanged
            nn.ReLU(inplace=True),
            nn.BatchNorm3d(out_channels*2),
            nn.Conv3d(out_channels*2, out_channels, kernel_size=3, stride=1, padding=1, bias=bias),
            nn.Dropout3d(p=0.5, inplace=True))
            # To Do  comment bias
        return layer
    ...
    def udecoder_1(self,in_channels,out_channels, bias=True):
        layer = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.BatchNorm3d(out_channels),
            nn.Conv3d(out_channels, out_channels, 3, stride=1, padding=1,bias=bias),
            nn.Dropout3d(p=0.5, inplace=True))
        return layer

    def udecoder_0(self, in_channels,out_channels,bias=True):
        layer = nn.Sequential(
            nn.ConvTranspose3d(in_channels,out_channels, 3, stride=1,padding=1,bias=bias),
            nn.Dropout3d(p=0.5, inplace=True))
        return layer
