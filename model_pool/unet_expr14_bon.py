...
class UNet3DB14(nn.Module):
    def __init__(self, in_channel, n_classes):
        self.in_channel = in_channel
        self.n_classes = n_classes
        super(UNet3DB14, self).__init__()
        self.ec0 = self.encoder(self.in_channel, 64, bias=True, batchnorm=True)
        self.dd1 = self.ddecoder_1(self.in_channel, 64, bias=True)
        self.dd2 = self.ddecoder_2(self.in_channel, 64, bias=True)
        ...
        self.cmid1 = self.cmid(64, 128, bias=True)
        self.cmid2 = self.cmid(128, 256, bias=True)
        self.cmid3 = self.cmid(128, 256, bias=True)

        self.pool2 = nn.MaxPool3d(2)
        self.ec6 = self.encoder(128, 256, bias=True, batchnorm=True)
        ...
        self.dc8 = self.decoder(256, 256, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.dc8_pu = self.decoder(256, 256, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.drop_low = torch.nn.Dropout3d(p=0.5, inplace=True)
        self.ud2 = self.udecoder_2(256, 128)  # in dc8_pu
        self.dc6 = self.decoder(256, 256, kernel_size=2, stride=2, bias=True, batchnorm=True)
        self.dc5 = self.decoder(256, 256, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.dc5_pu = self.decoder(256, 256, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.drop_mid = torch.nn.Dropout3d(p=0.5, inplace=True)
        self.ud1 = self.udecoder_1(256, 128)
        self.dc3 = self.decoder(256, 128, kernel_size=2, stride=2, bias=True, batchnorm=True)
        self.dc2_pu = self.decoder(128, 128, kernel_size=1, stride=1, padding=0, bias=True, batchnorm=True)
        self.drop_high = torch.nn.Dropout3d(p=0.5, inplace=True)
    ...
    def cmid(self, in_channels, out_channels, bias=True):
        layer = nn.Sequential(
            ...
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(out_channels, out_channels, 3, stride=1,
                      padding=1, bias=bias),
            nn.ReLU(inplace=True))
        return layer
    ...
    def gate_module(self, input1,input2,input3):
        ...
        input2_s = torch.max(softmax(input1,1),1,keepdim=True)[0]
