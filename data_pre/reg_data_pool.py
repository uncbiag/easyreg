...
class CustomDataSet(BaseRegDataSet):
            ...
            for j in range(num_img):
                if i!=j:
                    if self.label_path is not None:
                        img_pair_list.append([img_path_list[i],img_path_list[j],
    ...
    def gen_pair_dic(self):
        ...
        pair_list_dic = {sess: gen_pair_list_func(sub_patients_dic[sess], int(num_pair_limit * max_ratio[sess])) for
                         sess in sesses}
...
class RegDatasetPool(object):
    def create_dataset(self,dataset_name, sched, full_comb):
        self.dataset_dic = {'lpba':CustomDataSet,
                                    'oai':OaiDataSet}
