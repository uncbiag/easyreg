...
def do_pair_registration(index,pair_list, pair_name_list, weight_pair_list,do_affine=True,expr_folder=None,mermaid_setting_path=None):
    index =list(index)
    pair_list = [pair_list[i] for i in index]
    pair_name_list = [pair_name_list[i] for i in index]
    weight_pair_list = [weight_pair_list[i] for i in index] if weight_pair_list is not None else None
...
def demo_multi():
    ...
    parser.add_argument('--expr_name', required=False, default='non_name_0512',
    ...
    if not llf:
        ...
        if model_name == 'lddmm_adapt_smoother_map':
            ...
            mermaid_setting_path = '../mermaid/demos/sample_generation/cur_settings_lddmm_for_synth.json'
        else:
    ...
    num_weights =10
    batch_sz =2
    num_of_workers = 20  # for unknown reason, multi-thread not work
    ...
    total_pair_path_list, total_pair_name_list, total_init_weight_path_list = pair_path_list, pair_name_list, init_weight_path_list
    ...
    for i in range(num_stds):
        for j in range(num_weights):
            ...
            num_total_file = len(total_pair_name_list)
            total_index = list(range(num_total_file))
            get_mermaid_setting(mermaid_setting_path, expr_folder, modify=True, stds=stds, weights=weights)

            if num_of_workers > 1:
                splited_total_indexes = np.array_split(total_index, num_total_file//(num_of_workers*batch_sz))
                splited_total_indexes =list(splited_total_indexes)
                for splited_index in splited_total_indexes:
                    pair_path_list = [total_pair_path_list[i] for i in splited_index]
                    pair_name_list = [total_pair_name_list[i] for i in splited_index]
                    init_weight_path_list = [init_weight_path_list[i] for  i in splited_index] if total_init_weight_path_list is not None else None
                    if do_optimization:
                        num_files = len(pair_name_list)
                        indexes = np.array(range(num_files))
                        sub_p = partial(do_pair_registration, pair_list=pair_path_list, pair_name_list=pair_name_list,
                                        weight_pair_list=init_weight_path_list, do_affine=do_affine,
                                        expr_folder=expr_folder, mermaid_setting_path=mermaid_setting_path)
                        split_index = np.array_split(indexes, num_of_workers)
                        procs = []
                        for k in range(num_of_workers):
                            p = Process(target=sub_p, args=(split_index[k],))
                            p.start()
                ...
                do_pair_registration(total_index,total_pair_path_list, total_pair_name_list, total_init_weight_path_list, do_affine=do_affine,
                                     expr_folder=expr_folder, mermaid_setting_path=mermaid_setting_path)
            if do_evaluation:
                score, jacobi = analyze_on_pair_res(total_pair_path_list, total_pair_name_list, expr_folder=expr_folder, color_image=color_image)
