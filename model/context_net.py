...
class ContextNet(BaseModel):
    ...
    def initialize(self, opt):
        ...
        use_parallel = True
        self.net_f = networks.define_first_net(opt.input_nc, opt.output_nc, opt.ngf,
                                      opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids, use_parallel,
                                      opt.learn_residual)
        self.net_s = networks.define_second_net(opt.input_nc, opt.output_nc, opt.ngf,
                                      opt.which_model_netG, opt.norm, not opt.no_dropout, self.gpu_ids, use_parallel,
                                      opt.learn_residual)
        if not self.isTrain or opt.continue_train:
            self.load_network(self.net_f, 'f', opt.which_epoch)
            if self.isTrain:
                self.load_network(self.net_s, 's', opt.which_epoch)
